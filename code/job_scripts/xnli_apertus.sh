...
CONFIG_PATH="code/multilingual_transfer/configs/xnli_apertus.yaml"
# ==============================================

# Edit this for your project. Keep outputs/checkpoints under /scratch.
TRAIN_COMMAND="cd /scratch/${REPO_NAME} && pip install -r requirements.txt && pip install -e ./code/vllm_fuxitranyu && python code/multilingual_transfer/xnli_generate.py --config /scratch/${REPO_NAME}/${CONFIG_PATH}"
...
if [[ -z "${CLUSTER_FOLDER}" ]]; then
    echo "ERROR: CLUSTER_FOLDER environment variable is not set. Please define it in the .env file." >&2
    exit 1
fi
