...
Usage:
    python eclektic_correlate_collapsed.py \\
...
def _build_pred_df_for_ckpt_method(ckpt_method, geom, all_geom_ckpts, common_sorted, lang_codes):
    """Return pred_df for the given checkpoint collapse method (t=0, ECLeKTic has no time delay)."""
    if ckpt_method is None:
        return build_predictor_pairs(geom, common_sorted, lang_codes, t=0)
    elif ckpt_method == "average_rankme":
        rolled_geom = build_rolled_geom_avg_rankme(geom, all_geom_ckpts, common_sorted)
        return build_predictor_pairs(rolled_geom, common_sorted, lang_codes, t=0)
        ...
        if layer_method == "average_rankme":
            ...
            for ckpt_method in ckpt_collapse_methods:
                ...
                pred_df = _build_pred_df_for_ckpt_method(
                    ckpt_method, collapsed_geom, all_geom_ckpts, common_sorted, lang_codes
                )
            ...
            for ckpt_method in ckpt_collapse_methods:
                ...
                layer_preds = [
                    _build_pred_df_for_ckpt_method(
                        ckpt_method, layer_geoms[layer], all_geom_ckpts, common_sorted, lang_codes
                    )
                    for layer in selected_layers
                ]
