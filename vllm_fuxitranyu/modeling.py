...
class FuxiTranyuAttention(nn.Module):
    ...
    def _init_rotary_emb(
        ...
        if is_gguf and config.model_type == "fuxitranyu":
            is_neox_style = False
...
class FuxiTranyuDecoderLayer(nn.Module):
    def __init__(
        ...
        attn_layer_type: type[nn.Module] = FuxiTranyuAttention,
    ) -> None:
        super().__init__()
        ...
        self.mlp = FuxiTranyuMLP(
            hidden_size=self.hidden_size,
            intermediate_size=config.intermediate_size,
            hidden_act=getattr(config, "activation_function", "gelu_fast"),
            quant_config=quant_config,
            bias=True,
            prefix=f"{prefix}.mlp",
        )
...
class FuxiTranyuModel(nn.Module, EagleModelMixin):
    def __init__(
        ...
        *,
        vllm_config: VllmConfig,
        prefix: str = "",
        layer_type: type[nn.Module] = FuxiTranyuDecoderLayer,
    ):
        super().__init__()
...
class FuxiTranyuForCausalLM(
    ...
    def _init_model(
        ...
        return FuxiTranyuModel(vllm_config=vllm_config, prefix=prefix, layer_type=layer_type)
...
class FuxiTranyuBidirectionalForSequenceClassification(as_seq_cls_model(FuxiTranyuForCausalLM)):
    pass
